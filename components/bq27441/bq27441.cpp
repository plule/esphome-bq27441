...
namespace esphome::bq27441
{
    ...
    static const size_t MAX_STEP_RETRY = 20;
    static const uint32_t STEP_RETRY_TIME = 100;
    ...
    void BQ27441::setup_begin()
    {
        ...
        this->setup_write_config();
    }

    void BQ27441::setup_write_config(size_t attempt)
    {
        ESP_LOGV(TAG, "Checking if the device is in config mode %d/%d", attempt + 1, MAX_STEP_RETRY);
        ...
        {
            // Not yet in cfg update, schedule next check or fail
            if (attempt <= MAX_STEP_RETRY)
                this->set_timeout(STEP_RETRY_TIME, [this, attempt]()
                                  { this->setup_write_config(attempt + 1); });
            else
        ...
        // Read the previous data checksum to update it
        ESP_LOGV(TAG, "Read previous block RAM checksum");
        ...
        if (changed)
        {
            ESP_LOGI(TAG, "Performing soft reset after configuration update");
            this->write_u16(BQ27441_COMMAND_CONTROL, BQ27441_CONTROL_SOFT_RESET);
        }
        else if (itpor)
        {
            ESP_LOGI(TAG, "Performing soft reset after powerloss");
            this->write_u16(BQ27441_COMMAND_CONTROL, BQ27441_CONTROL_SOFT_RESET);
        }
        else
        {
        ...
        this->setup_exit_config();
    }

    void BQ27441::setup_exit_config(size_t attempt)
    {
        ESP_LOGV(TAG, "Checking if the device is out of config mode %d/%d", attempt + 1, MAX_STEP_RETRY);

        optional<uint16_t> flags_value = this->read_u16(BQ27441_COMMAND_FLAGS);
        if (!flags_value.has_value() || (flags_value.value() & BQ27441_FLAG_CFGUPMODE))
        {
            // Not yet out of config mode, schedule next check or fail
            if (attempt <= MAX_STEP_RETRY)
                this->set_timeout(STEP_RETRY_TIME, [this, attempt]()
                                  { this->setup_exit_config(attempt + 1); });
            else
